...
## 概述
本项目是一个探索Linux内核最新异步I/O接口`io_uring`极限性能的原型库。通过结合**Thread-Per-Core线程模型**、**无锁SPSC/MPSC队列**及**缓存行对齐**等技术，构建了一个从用户态到内核态完全异步、无锁竞争的高并发I/O管道。
...
2、**新增批内核大页机制**：在批处理的基础上新增内核大页分配。通过合理的内存管理，增加约 **17%** 的整体吞吐量，详情见docs/huge_page.md，减少了约  **14.7%** 的时间开销。
